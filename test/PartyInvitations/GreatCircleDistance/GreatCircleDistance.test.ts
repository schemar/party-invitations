...
describe('GreatCircleDistance', () => {
  ...
  // Add more test cases in a prod environment, with more edge cases close to the min/max lat/lon.
    ...
    test.each(distanceCases)('Should calculate the correct distance: %p', ({positionOne, positionTwo, expectedDistanceInKm}) => {
        // Arrange
        const greatCircleDistance = GreatCircleDistance.newEarth();

      // Act
        const distance = greatCircleDistance.distance({positionOne, positionTwo});
